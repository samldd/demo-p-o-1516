...
img = cv2.imread('last_image (3).jpg')
...
cv2.createTrackbar('Vmin','image',229,255,nothing)
...
while(1):
    ...
    im = cv2.medianBlur(img[len(img)/3:,:],31)
    im = cv2.medianBlur(im,31)
    im = cv2.inRange(im, np.array((Hmin, Smin, Vmin)), np.array((Hmax, Smax, Vmax)))
